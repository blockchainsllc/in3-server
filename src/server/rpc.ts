...
export async function handle(request: RPCRequest[]): Promise<RPCResponse[]> {
  return Promise.all(request.map(r => {
    const in3Request = r.in3 || { chainId: util.toHex((config.chainIds && config.chainIds[0]) || '0x2a', 32) }
...
export function updateNodelists() {
  return Promise.all(config.chainIds.map(id => handlers[util.toHex(id, 32)].getNodeList(true)))
}
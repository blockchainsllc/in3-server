...
export class TestTransport implements Transport {
  ...
  async getServerFromContract(index: number) {
    const [url, owner, timeout, deposit, props, unregisterTime, unregisterDeposit, unregisterCaller] = await callContract(this.url, this.nodeList.contract, 'servers(uint):(string,address,uint64,uint,uint,uint128,uint128,address)', [index])
    return { url, owner, timeout, deposit, props, unregisterTime, unregisterDeposit, unregisterCaller }
  }
